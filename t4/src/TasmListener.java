...
public class TasmListener extends TasmBaseListener {
    ...
    public void labelLink(){
        for (int i = 0; i<instructions.size();i++) {
            ...
            if (instruction.getOp().equals(TokenTasm.JUMP) || instruction.getOp().equals(TokenTasm.JUMPT) || instruction.getOp().equals(TokenTasm.JUMPF)) {
                String label = jumpLabels.get(instruction.getArgument()-1);
                Integer l = labels.get(label);
                if(l!=null)
                    instruction.setArgument(l);
                else
